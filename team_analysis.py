...
def team_summary(analzyed_data):
    team_summary = analzyed_data.groupby('team.number').agg(
        ...
        avg_disabled_seconds=('robot.disabled.time', 'mean'),
        rps=('rp.pts', 'sum'),
    ...
    team_summary['rank_by_avg_pts'] = team_summary[['avg_total_pts', 'avg_teleop']].apply(tuple, axis=1).rank(
    ...
    team_summary['total_fouls'].fillna(0)
    team_summary['avg_disabled_seconds'].fillna(0)
    # teleop accuracies
    # TODO: duplicated for individual team in compute_scoring_table
    team_summary['subwoofer_accuracy_teleop'] = team_summary['speaker_subwoofer_completed_teleop'] / (
                team_summary['speaker_subwoofer_attempted_teleop'] + team_summary['speaker_subwoofer_completed_teleop'])
    team_summary['podium_accuracy_teleop'] = team_summary['speaker_podium_completed_teleop'] / (
                team_summary['speaker_podium_attempted_teleop'] + team_summary['speaker_podium_completed_teleop'])
    team_summary['amp_accuracy_teleop'] = team_summary['amp_completed_teleop'] / (
                team_summary['amp_attempted_teleop'] + team_summary['amp_completed_teleop'])

    team_summary['subwoofer_accuracy_auto'] = team_summary['speaker_subwoofer_completed_auto'] / (
                team_summary['speaker_subwoofer_attempted_auto'] + team_summary['speaker_subwoofer_completed_auto'])
    team_summary['podium_accuracy_auto'] = team_summary['speaker_podium_completed_auto'] / (
                team_summary['speaker_podium_attempted_auto'] + team_summary['speaker_podium_completed_auto'])
    team_summary['amp_accuracy_auto'] = team_summary['amp_completed_auto'] / (
                team_summary['amp_attempted_auto'] + team_summary['amp_completed_auto'])
...
def team_analyze(all_data):
    ...
    base_data["notes.speaker.auto"] = base_data["speaker.subwoofer.completed.auto"] + base_data[
        "speaker.podium.completed.auto"] + base_data["speaker.medium.completed.auto"]
    base_data["notes.speaker.teleop"] = base_data["speaker.subwoofer.completed.teleop"] + base_data[
        "speaker.podium.completed.teleop"] + base_data["speaker.medium.completed.teleop"]
