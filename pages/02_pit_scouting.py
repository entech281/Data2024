...
with pit_form:
    ...
    with col1:
        record.robot_length = st.number_input(label="Robot Length (in.)", min_value=0.0, max_value=40.0, value=0.0,
                                              key="robot_length")
        record.robot_width = st.number_input(label="Robot Width (in.)", min_value=0.0, max_value=40.0, value=0.0,
                                             key="robot_width")
        record.robot_height = st.number_input(label="Robot Starting Height (in.)", min_value=0.0, max_value=60.0,
                                              value=0.0, key="robot_height")

    with col2:
        record.robot_weight = st.number_input(label="Robot Weight (lbs.)", min_value=0.0, max_value=125.0, value=0.0,
                                              key="robot_weight")
    ...
    with col1:
        record.pref_start = st.multiselect("Preferred Starting Locations", key="pref_starts", max_selections=3,
                                           options=StartLocEnum.options())

        record.robot_pickup = st.radio("Preferred Pickup Locations", key="robot_pickup", index=0,
                                       options=PickupEnum.options())

    with col2:
        record.score_abilities = st.multiselect("Scoring Abilities", key="robot_scoring_abilities", max_selections=5,
                                                options=ShotLocEnum.options())

        record.pref_shoot = st.multiselect("Preferred Scoring Locations", key="pref_shots", max_selections=5,
                                           options=ShotLocEnum.options())
